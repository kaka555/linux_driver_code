...
static void tiny_timer(unsigned long timer_data)
{
	...
	for (i = 0; i < data_size; ++i) {
		int num = tty_insert_flip_char(tty, data[i], TTY_NORMAL);
		printk("tty_insert_flip_char num is %d\n",num);
		if ((++tiny->rx_count % 10) == 0)
			tty_insert_flip_char(tty, '\n', TTY_NORMAL);
	}
	tty_flip_buffer_push(tty);
	printk("finish tty_flip_buffer_push()\n");
...
static int __init tiny_init(void)
{
	...
	tiny_tty_driver = alloc_tty_driver(TINY_TTY_MINORS);/* 参数是申请的设备数量 */
	if (!tiny_tty_driver)
		return -ENOMEM;
	...
	tiny_tty_driver->major = TINY_TTY_MAJOR, //240
	...
	for (i = 0; i < TINY_TTY_MINORS; ++i)
		tty_register_device(tiny_tty_driver, i, NULL);/* 第二个参数必须小于alloc_tty_driver传入的参数，即driver->num */
